...
class SmartProcessingQueue(_VariablePriorityQueueMixin[T], ProcessingQueue[Concatenate[T, P], V]):
    ...
    def put_nowait(self, *args: P.args, **kwargs: P.kwargs) -> SmartFuture[V]:
        ...
        if fut is None:
            ...
            self._futs[key] = fut
            # we need a strong ref to the fut at least until the callbacks run,
            # even if the outer task is cancelled or garbage collected
        ...
        def pop_pending(f: SmartFuture[Any]) -> None:
