...
async def gather(*awaitables: Awaitable[T], return_exceptions: bool = False, tqdm: bool = False, **tqdm_kwargs: Any) -> List[T]:
    return await (
        gather_mapping(awaitables[0], return_exceptions=return_exceptions, tqdm=tqdm, **tqdm_kwargs) if len(awaitables) == 1 and isinstance(awaitables[0], Mapping)
...
async def gather_mapping(mapping: Mapping[KT, Awaitable[VT]], return_exceptions: bool = False, tqdm: bool = False, **tqdm_kwargs: Any) -> Dict[KT, VT]:
    ...
    async for k, v in as_completed_mapping(mapping, return_exceptions=return_exceptions, tqdm=tqdm, **tqdm_kwargs):
        results[k] = v
    return results