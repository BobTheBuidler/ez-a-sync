...
if TYPE_CHECKING:
    ...
    class ASyncDescriptor(ModifiedMixin, Generic[T]):
        def __init__(self, _fget: UnboundMethod[P, T], field_name=None, **modifiers: ModifierKwargs):
            if not callable(_fget):
                raise ValueError(f'Unable to decorate {_fget}')
            self.modifiers = ModifierManager(modifiers)
            self._fn: UnboundMethod[P, T] = _fget
            self._fget: AsyncUnboundMethod[P, T] = self.modifiers.apply_async_modifiers(_fget) if asyncio.iscoroutinefunction(_fget) else self._asyncify(_fget)
