...
class ASyncIterable(AsyncIterable[T], Iterable[T]):
    """
    Docs: 
        [https://bobthebuidler.github.io/ez-a-sync/source/a_sync.html#a_sync.iter.ASyncIterable](https://bobthebuidler.github.io/ez-a-sync/source/a_sync.html#a_sync.iter.ASyncIterable)
    ...
    Attributes:
        __wrapped__ (AsyncIterable[T]): The wrapped async iterable object.
    
    Methods:
        wrap(cls, wrapped: AsyncIterable[T]) -> "ASyncIterable[T]": Class method to wrap an AsyncIterable for backward compatibility.
        __init__(self, async_iterable: AsyncIterable[T]): Initializes the ASyncIterable with an async iterable.
        __iter__(self) -> Iterator[T]: Returns an iterator for the wrapped async iterable.
        __aiter__(self) -> AsyncIterator[T]: Returns an async iterator for the wrapped async iterable.
    """
    @classmethod
...
class ASyncIterator(AsyncIterator[T], Iterator[T]):
    """
    Docs: 
        [https://bobthebuidler.github.io/ez-a-sync/source/a_sync.html#a_sync.iter.ASyncIterator](https://bobthebuidler.github.io/ez-a-sync/source/a_sync.html#a_sync.iter.ASyncIterator)
    ...
    Attributes:
        __wrapped__ (AsyncIterator[T]): The wrapped async iterator object.
    
    Methods:
        wrap(cls, wrapped): Class method to wrap either an AsyncIterator or an async generator function.
        __init__(self, async_iterator: AsyncIterator[T]): Initializes the ASyncIterator with an async iterator.
        __anext__(self) -> T: Asynchronously fetches the next item from the async iterator.
        __next__(self) -> T: Fetches the next item from the async iterator synchronously.
    """
    def __next__(self) -> T:
        try:
...
class ASyncGeneratorFunction(Generic[P, T]):
    """
    Docs: 
        [https://bobthebuidler.github.io/ez-a-sync/source a_sync.html#a_sync.iter.ASyncGeneratorFunction](https://bobthebuidler.github.io/ez-a-sync/source/a_sync.html#a_sync.iter.ASyncGeneratorFunction)
    ...
    Attributes:
        field_name (str): The name of the async generator function.
        __wrapped__ (AsyncGenFunc[P, T]): The actual async generator function.
        __instance__ (Any): The instance the function is bound to, if any.
    
    Methods:
        __init__(self, async_gen_func: AsyncGenFunc[P, T], instance: Any = None): Initializes the ASyncGeneratorFunction with the given async generator function and optionally an instance.
        __call__(self, *args: P.args, **kwargs: P.kwargs) -> ASyncIterator[T]: Calls the wrapped async generator function with the given arguments and keyword arguments, returning an ASyncIterator.
        __get__(self, instance: Any, owner: Any) -> "ASyncGeneratorFunction[P, T]": Descriptor method to make the function act like a non-data descriptor.
        __repr__(self) -> str: Returns the representation of the ASyncGeneratorFunction.
    """
