...
class TaskMapping(DefaultDict[K, "asyncio.Task[V]"], AsyncIterable[Tuple[K, V]]):
    ...
    def __init__(
        ...
        **wrapped_func_kwargs: P.kwargs,
        ...
        if iterables:
            self._next = Event(name="{} `_next`".format(self))
...
cdef void _raise_if_not_empty(object mapping):
    if mapping:
        raise exceptions.MappingNotEmptyError(mapping)
